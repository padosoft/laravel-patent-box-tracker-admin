...
2. **All** CI checks on the PR head commit are passing — `gh pr checks <PR>` reports each row as `pass`. **No row may be `pending`, `fail`, or `skipping`** (whether the skipped check is "required" or not is irrelevant — the strict gate below rejects any non-`SUCCESS` conclusion). The equivalent JSON form is `gh pr view <PR> --json statusCheckRollup` returning a **non-empty** array where every entry has `status=="COMPLETED"` and `conclusion=="SUCCESS"`. The `length > 0` guard is non-negotiable: `[ ] | all` evaluates to `true` in jq, so an empty rollup would otherwise satisfy the check vacuously. Use the full expression `(.statusCheckRollup | length > 0) and ([.statusCheckRollup[] | (.status=="COMPLETED" and .conclusion=="SUCCESS")] | all)` and require the result to be `true`.
3. The latest Copilot review **for the current PR head commit** has **0 inline review comments** AND its body indicates no new comments (e.g. "generated no new comments"), OR Copilot has explicitly approved the PR (review state `APPROVED`). The filter applied to `gh api .../pulls/{n}/reviews` is non-negotiable and has three predicates that must all hold:
   - **login**: `.user.login == "copilot-pull-request-reviewer[bot]"` — this is the canonical login for the GitHub Copilot review bot when it submits a review on this repo, and the value is locked. **GitHub returns two different login strings for the same Copilot bot in two different endpoints**: `gh api repos/<owner>/<repo>/pulls/<n>/requested_reviewers --jq '.users[].login'` returns `"Copilot"` (the value used to register the review request), while `gh api repos/<owner>/<repo>/pulls/<n>/reviews --jq '.[].user.login'` returns `"copilot-pull-request-reviewer[bot]"` (the value used when the bot actually submits a review). The convergence filter operates on `/reviews`, so the locked string is `copilot-pull-request-reviewer[bot]`; the `requested_reviewers` check (used only to confirm the reviewer was attached after `gh pr edit --add-reviewer @copilot`) should match `"Copilot"` instead. A human reviewer or a third-party bot on HEAD MUST NOT be treated as the Copilot quiet/approval signal;
