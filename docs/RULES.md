...
3. The latest Copilot review **for the current PR head commit** has **0 inline review comments** AND its body indicates no new comments (e.g. "generated no new comments"), OR Copilot has explicitly approved the PR (review state `APPROVED`). The head-SHA constraint is non-negotiable: filter `gh api .../pulls/{n}/reviews` by `commit_id == headRefOid` AND a state-whitelist (`APPROVED` or `COMMENTED` — never `PENDING` and never `DISMISSED`) before evaluating the inline-count/body/state. A stale review of an older commit, a pending review, or a review that has been dismissed by a maintainer MUST NOT satisfy this gate.
