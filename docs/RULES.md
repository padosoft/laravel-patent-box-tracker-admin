...
3. The latest Copilot review **for the current PR head commit** has **0 inline review comments** AND its body indicates no new comments (e.g. "generated no new comments"), OR Copilot has explicitly approved the PR (review state `APPROVED`). The filter applied to `gh api .../pulls/{n}/reviews` is non-negotiable and has three predicates that must all hold:
   - **login**: `.user.login == "copilot-pull-request-reviewer[bot]"` — this is the canonical login for the GitHub Copilot review bot installed on this repo and is locked. To verify on a fresh clone, run `gh api repos/<owner>/<repo>/pulls/<n>/requested_reviewers --jq '.users[].login'` after `gh pr edit <n> --add-reviewer @copilot`; the value returned MUST be exactly `Copilot` (display name) which submits reviews under the login `copilot-pull-request-reviewer[bot]`. A human reviewer or a third-party bot on HEAD MUST NOT be treated as the Copilot quiet/approval signal;
